...
class AsyncioTelnetServer:
    ...
    def _get_reader(self, network_reader):
        ...
        with (yield from self._lock):
            ...
            if self._reader:
                if self._reader_process == network_reader:
                    self._current_read = asyncio.coroutine(self._reader.read(READ_SIZE))
...
if __name__ == '__main__':
    ...
    process = loop.run_until_complete(asyncio.coroutine(asyncio.subprocess.create_subprocess_exec("/bin/sh", "-i",
